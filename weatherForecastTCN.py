...
from tcn import TCN, tcn_full_summary  # Import TCN from keras-tcn library
...
CHOSEN_MODEL = "TCN"    # Choose which model structure to use
EPOCHS = 50             # Training epochs
BATCH_SIZE = 16         # Batch size
DROPOUT = 0.2           # Dropout
LEARNING_RATE = 0.001   # Learning rate
HIDDEN_SIZE = 50        # Number of TCN filters
...
# Reshape data to fit TCN input and print progress
print_progress("Reshaping data")
X_train = X_train.reshape(X_train.shape[0], X_train.shape[1], 1)

# Build TCN model using functional API
print_progress("Building model")
input_layer = Input(shape=(TIME_STEP, 1))
tcn_layer = TCN(nb_filters=HIDDEN_SIZE, kernel_size=3, nb_stacks=1, dilations=[1, 2, 4, 8], padding='causal', use_skip_connections=True, dropout_rate=DROPOUT, return_sequences=False)(input_layer)
dense_layer1 = Dense(25)(tcn_layer)
...
# Reshape prediction data to fit TCN input and print progress
...
plt.plot(combined_data.index[TIME_STEP + 1:], predictions, label='Predicted Temperature with TCN')
plt.plot(data_2023['datetime'], data_2023['temp'], label='Actual Temperature')
plt.xlabel('Date')
plt.ylabel('Temperature')
plt.title('Temperature Prediction with TCN')
plt.legend()
...
plt.savefig('result/temperature_prediction_TCN.png')
plt.show()
...
plt.xlabel('Metrics')
plt.ylabel('Value')
plt.title('Prediction Metrics with TCN')

# Save the metrics plot
plt.savefig('result/prediction_metrics_TCN.png')
plt.show()
...
with open('result/error_metrics_TCN.json', 'w') as json_file:
