...
# Reshape data to fit LSTM input and print progress
print_progress("Reshaping data")
X_train = X_train.reshape(X_train.shape[0], X_train.shape[1], 1)

# Build LSTM model and print progress
...
# Reshape prediction data to fit LSTM input and print progress
...
plt.plot(combined_data.index[TIME_STEP + 1:], predictions, label='Predicted Temperature with LSTM')
...
plt.savefig('result/temperature_prediction_LSTM.png')
plt.show()
...
plt.xlabel('Metrics')
plt.ylabel('Value')
plt.title('Prediction Metrics with LSTM')

# Save the metrics plot
plt.savefig('result/prediction_metrics_LSTM.png')
plt.show()
...
with open('result/error_metrics_LSTM.json', 'w') as json_file:
